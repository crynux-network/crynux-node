...
- **Joined-node capability synchronization**: when startup reads a relay status showing that the node remains joined, the node MUST send one authenticated capability report containing the current GPU name, GPU VRAM, complete local on-disk model ID set, and runner version. Startup MUST send the same report after resuming a paused node. This synchronization repairs relay scheduling state after the same node address moves to another machine or starts with a recreated local model cache without first quitting.
...
1. The local task record (local database) holds the persisted artifacts of every known task, written on every change: status, result files, score, checkpoint, optional Worker-reported execution dtype, and result-uploaded marker.
2. The first reconcile cycle after startup is identical to every other cycle: it queries the node's current task and derives the action from the fresh status plus the persisted artifacts. A valid persisted score means the worker phase is complete and MUST NOT be re-run; score submission after recovery MUST use the persisted execution dtype when present. A non-terminal task without a score repeats its task-scoped non-base-model downloads and inference on the new worker processes, as permitted by the one-shot rule in `docs/task-lifecycle.md`. The server MUST dispatch the downloads without inspecting the Hugging Face cache and MUST rely on worker idempotence.
3. The event cursor restarts at the current event id. Recovery MUST NOT expect any event from before the restart.
4. If the node remains joined, startup MUST complete joined-node capability synchronization before entering normal status polling. A failed synchronization MUST keep retrying through the startup retry path and MUST NOT allow execution against stale relay capability state.

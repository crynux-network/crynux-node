...
- The **event system** carries only flows that have no query equivalent (model download commands) and display hints. No correctness-critical inference task behavior depends on event delivery.
...
- `DownloadModel`: create a model download task. This is the only channel for download commands — the relay offers no query for pending downloads — and the reason the event watcher exists. Downloads are a best-effort locality optimization, so commands lost while the node is offline are accepted losses. Download task creation is idempotent against duplicate delivery.
...
- All inference task behavior: discovery, execution, error report, score submission, result upload, and local closure — every action of the reconcile loop.
- Node status, scores, and staking state.

### Decisions owned by neither channel

- Worker process health and restart. The worker manager decides restarts exclusively from task execution outcomes it observes directly (result type and execution deadline). See `docs/task-system-design.md`.
...
- Event callbacks MUST be short and non-blocking with respect to the watcher loop; long-running work triggered by an event MUST be spawned into the owning component (download task creation registers the task and returns).
